...
func run(w io.Writer) int {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "codecovcheck:", err) // # pragma: no cover — findRepoRoot's own error branches are pragma'd below; this only wraps that failure for the CLI entry point, which tests bypass via runAt
