...
// Sentinel errors; services/monica_import_session.go maps these to
// user-facing apperrors and log-safe messages that never carry the token.
var (
...
// DisableRateLimitForTesting so test suites do not wait a second per request.
...
// connections to private/loopback addresses are refused via the shared
// httputil.SafeDialContext guard (SSRF protection for cloud deployments;
// self-hosted Monica usually lives on a LAN, so this is opt-in — the same
// trade-off IMMICH_BLOCK_PRIVATE_URLS / PAPERLESS_BLOCK_PRIVATE_URLS make).
func NewClient(baseURL, token string, blockPrivate bool) (*Client, error) {
	...
	if blockPrivate {
		// The dialer re-resolves the host and pins a public IP, closing both
		// DNS-rebinding and redirect-to-internal bypasses (see
		// httputil.SafeDialContext's doc comment).
		transport.DialContext = httputil.SafeDialContext(
			fmt.Errorf("%w: could not resolve host", ErrUnreachable),
			ErrPrivateAddress,
		)
	}
	return &Client{
...
// response into out, retrying 429s per Retry-After and 5xx up to maxRetries.
...
// page with (items fetched so far, meta total).
...
// FetchAllActivities pulls every activity.
...
// FetchAllNotes pulls every note.
...
// FetchAllReminders pulls every reminder.
...
// FetchAllCalls pulls every logged call.
...
// FetchAllTasks pulls every task.
...
// FetchAllGifts pulls every gift.
...
// FetchAllDebts pulls every debt.
