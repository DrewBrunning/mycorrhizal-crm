...
// correspondence.Load() and asserts that every concept with a vCard3 home
...
func TestCoverage_AllMappedConceptsHaveImportAndExportTests(t *testing.T) {
	...
	for _, row := range correspondence.Load() {
		...
		if !importCoverage[row.ConceptID] {
			t.Errorf("concept %q (v3_prop %q) has no registered import test coverage", row.ConceptID, row.V3Prop)
		}
		if !exportCoverage[row.ConceptID] {
			t.Errorf("concept %q (v3_prop %q) has no registered export test coverage", row.ConceptID, row.V3Prop)
		}
	}
}
