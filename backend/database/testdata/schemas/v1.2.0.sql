-- Schema-only dump for release v1.2.0 (migration version 66).
-- Generated by `cd backend && go run ./cmd/genschema -tag v1.2.0 -version 66`
-- from the embedded migration chain (frozen and append-only, so migrations
-- 000001..000066 are byte-identical to the tagged release's own files).
...
, notify_ntfy INTEGER NOT NULL DEFAULT 0, notify_gotify INTEGER NOT NULL DEFAULT 0, notify_push INTEGER NOT NULL DEFAULT 0, self_contact_vcard_uid TEXT, totp_secret_encrypted TEXT, totp_enabled INTEGER NOT NULL DEFAULT 0, totp_confirmed_at DATETIME, totp_last_used_step BIGINT);
...
CREATE TABLE reminders (
    ...
    life_event_id TEXT, revision INTEGER NOT NULL DEFAULT 1, etag TEXT, occasion_obligation_id TEXT,
    FOREIGN KEY (contact_id) REFERENCES contacts(id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE calendar_subscriptions (
    ...
    last_sync_error TEXT NOT NULL DEFAULT '', last_attempt_at DATETIME, last_success_at DATETIME, last_failure_at DATETIME, consecutive_failures INTEGER NOT NULL DEFAULT 0, incident_first_failure_at DATETIME, last_run_duration_ms INTEGER, last_run_stats TEXT NOT NULL DEFAULT '', terminal_failure_at DATETIME, terminal_reason TEXT NOT NULL DEFAULT '',
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);
...
CREATE TABLE life_events (
    ...
    etag TEXT, category TEXT, revision INTEGER NOT NULL DEFAULT 1, end_date TEXT,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);
...
CREATE TABLE field_definitions (
    ...
    sensitivity TEXT NOT NULL DEFAULT 'normal', position INTEGER NOT NULL DEFAULT 0,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);
...
CREATE TABLE preferences (
    ...
    sensitivity TEXT NOT NULL DEFAULT 'normal', notes TEXT, level TEXT,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);
...
CREATE TABLE webhook_deliveries (
    ...
    next_retry_at DATETIME, failed_permanently INTEGER NOT NULL DEFAULT 0, terminal_reason TEXT NOT NULL DEFAULT '',
...
CREATE INDEX idx_contacts_user_vcard_uid_all
    ON contacts(user_id, vcard_uid);
...
CREATE UNIQUE INDEX idx_relationship_edges_natural_key
...
INSERT INTO schema_migrations (version, dirty) VALUES (66, 0);
