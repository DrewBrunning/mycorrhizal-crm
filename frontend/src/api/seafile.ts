...
export async function getSeafileLibraries(): Promise<SeafileLibrary[]> {
  const response = await apiFetch(`${API_BASE_URL}/seafile/libraries`, { headers: getAuthHeaders() });
  if (!response.ok) throw await parseErrorResponse(response);
...
export async function getSeafileDir(repoId: string, path: string): Promise<SeafileItem[]> {
  ...
  const response = await apiFetch(`${API_BASE_URL}/seafile/libraries/${encodeURIComponent(repoId)}/dir?${params.toString()}`, {
...
export async function linkSeafileItem(
  contactUid: string,
  item: { repo_id: string; path: string; name: string; type: 'file' | 'dir'; size?: number; mtime?: number }
): Promise<void> {
...
export async function unlinkSeafileItem(contactUid: string, identityId: string): Promise<void> {
  const response = await apiFetch(`${API_BASE_URL}/seafile/contacts/${contactUid}/links/${identityId}`, {
    method: 'DELETE', headers: getAuthHeaders(),
  });
  if (!response.ok) throw await parseErrorResponse(response);
}
