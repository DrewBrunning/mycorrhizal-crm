...
describe('extractDetailMessage', () => {
  ...
  test('joins string values from the details object', () => {
    expect(
      extractDetailMessage({ email: 'Email already taken', username: 'Username too short' }),
    ).toBe('Email already taken Username too short');
  });
...
describe('handleResponse', () => {
  ...
  test('handles the simple string error format', async () => {
    await expect(
      handleResponse(jsonResponse({ error: 'Something broke' }, 500), 'fallback'),
    ).rejects.toThrow('Something broke');
  });

  test('uses a top-level message field when present', async () => {
    await expect(
      handleResponse(jsonResponse({ message: 'Too many requests' }, 429), 'fallback'),
  ...
  test('falls back to the provided message for an empty error body', async () => {
    await expect(
      handleResponse(new Response('', { status: 500 }), 'Request failed'),
