...
function renderTimeline(
  items: React.ComponentProps<typeof ContactTimeline>['timelineItems']
) {
  return render(
    <MemoryRouter>
      <DateFormatProvider>
...
test('renders a received gift with its own label', () => {
  renderTimeline([
    { type: 'gift', data: { ...givenGift, status: 'received', description: 'The scarf' }, date: givenGift.date! },
  ]);
...
test('a gift without an occasion does not render the occasion line', () => {
  ...
  renderTimeline([{ type: 'gift', data: { ...rest, description, occasion: undefined }, date: givenGift.date! }]);
