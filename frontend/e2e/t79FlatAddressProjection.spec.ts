...
import { waitForLoading, deleteTestContact } from './fixtures';
...
function subStreetPayload(firstname: string, apartmentToken: string) {
  return {
    card: {
      name: { components: [{ kind: 'given', value: firstname }, { kind: 'surname', value: 'T79' }] },
      addresses: [
        {
          components: [
...
async function createSubStreetContact(
  ...
  expect(res.ok(), `failed to create T79 test contact: ${res.status()} ${await res.text()}`).toBeTruthy();
...
test.describe('T79: flat address projection carries PO box / apartment / floor', () => {
  test('displays the sub-street parts in the contact detail address line', async ({ page, request }) => {
