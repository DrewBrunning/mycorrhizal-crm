...
// FIELDS), so the spec enables it for the shared test user up front and
// restores the default set afterwards.
const DEFAULT_ENABLED_FIELDS = [
  'emails', 'phones', 'addresses', 'nickname', 'gender', 'birthday',
  'speakToAs', 'personalInfo', 'how_we_met', 'work_information', 'contact_information',
];
...
test.describe('Date format selection (v0.4.1)', () => {
  test('lists the built-out formats and renders birthdays + anniversaries in the chosen one', async ({ page, request }) => {
    ...
    const enableResp = await request.patch(`${API_BASE_URL}/users/enabled-contact-fields`, {
      data: { fields: [...DEFAULT_ENABLED_FIELDS, 'anniversary', 'anniversaries'] },
    });
    ...
    try {
      ...
      await request.patch(`${API_BASE_URL}/users/enabled-contact-fields`, {
        data: { fields: DEFAULT_ENABLED_FIELDS },
      }).catch(() => {});
    }
  });
});
