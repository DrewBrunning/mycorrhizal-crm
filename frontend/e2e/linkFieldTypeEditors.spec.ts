...
//
// The toggle is safe against the parallel specs that share TEST_USER (see
// contactFieldLinking.spec.ts's note): no other spec reads or writes
// /users/enabled-contact-fields, and T30's emptiness gate means turning a
// field on only adds a section when a contact actually has data for it —
// other specs' fixtures have none, so their rendered layout is unchanged.
