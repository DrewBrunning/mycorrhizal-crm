...
|---|---|---|---|
| **Go server binary** | **Yes — byte-identical** | Pinned Go toolchain (`backend/go.mod` `toolchain`), `CGO_ENABLED=0` pure-Go build, `-trimpath` (drops the builder's absolute paths), `-buildvcs=false` (keeps the embedded VCS stamp out — the commit is carried by an `-X` ldflag instead), `SOURCE_DATE_EPOCH` honoured for any embedded time. `.github/workflows/reproducibility.yml` → `go-binary` builds it twice from two different paths on the same runner and `cmp`s the result — that proves path-independence, and is a required check on every PR; the actual double build only runs when the PR touches the backend/build inputs (path-filtered via the `changes` job and the `repro`/`workflows` filters in `.github/filters.yaml`) — otherwise the job still runs and reports so a required check can never be stranded waiting on a status a path filter prevented from ever posting. `go-binary-independent-rebuild` + `-compare` go further with a true independent rebuild: two different GitHub-hosted runner images, built and compared independently — the check that actually substantiates "byte-reproducible" end-to-end — but only weekly (and on manual dispatch), not per-PR (issue #947). | `/health` → `commit` (`backend/buildinfo/buildinfo.go`), a 12-char prefix; cross-check per [release-verification.md](release-verification.md#verifying-sourcerelease-correspondence). |
| **All-in-one image** (`linux/amd64`) | **Config + layer digests compared every run; not yet a hard gate** | Base images pinned **by digest** (`golang:1.27.1-alpine@sha256:…`, `node:26-alpine@sha256:…`, `alpine:3.24@sha256:…`), apk packages pinned to exact `=version-rN`, `-trimpath -buildvcs=false` on the Go build, and `SOURCE_DATE_EPOCH` (the release commit's committer date) passed to BuildKit so every layer/config timestamp is rewritten to it. The BuildKit builder image itself is now pinned too (issue #1109) — `docker/setup-buildx-action`'s `driver-opts: image=moby/buildkit:buildx-stable-1@sha256:…` in place of the previously-floating default tag, so a resolve-time BuildKit version drift can no longer be the cause of a digest mismatch. `reproducibility.yml` → `all-in-one-image` double-builds it and records the config + layer-digest comparison in the job summary every run; it is `continue-on-error` until it has been green across enough `main` runs to block a merge on (the `apk add` layer's one source of non-determinism, `/var/log/apk.log`, is removed in the `Dockerfile`; pinning the builder image closes the last *unpinned* input, though it alone doesn't prove any given flake was caused by it — that needs repeated green runs). | `org.opencontainers.image.revision` label (full SHA, set explicitly from the checked-out tag — not `github.sha`), and `/health` from the running backend. |
...
| **Android release APK** | **No — not bit-reproducible by design** | The APK is signed with the release keystore (`SIGNING_*` secrets); the signature block differs every build by construction. R8 shrinking, dexing, and zipalign are also not guaranteed identical across Android toolchain patch versions. **Mitigation:** the keystore signature (Android's own trust mechanism — proves the same key built every release) plus GitHub-native SLSA provenance (`gh attestation verify app-obtainium-release.apk`) naming the source commit and workflow run. | SLSA provenance predicate (`gh attestation verify …`). |
