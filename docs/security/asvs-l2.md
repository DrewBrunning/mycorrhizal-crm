...
| 1.14.2 | Binary signatures / verified endpoints | satisfied | cosign-signed images + SLSA provenance (`docker-publish.yml:1068-1074`, `docker-publish.yml:1094-1102`); released images additionally carry cosign-signed portable Syft SBOMs, SPDX + CycloneDX (`docker-publish.yml:1150-1156`, `syft-sbom.yml`); the release APK is cosign co-signed keyless (additive to the SIGNING_* keystore) alongside `attest-build-provenance` (`docker-publish.yml:611-614,647-649`); operator-facing verification steps (issue #418) — `docs/security/release-verification.md`, whose `cosign verify` commands **pin `--certificate-identity-regexp` to the release workflow on a tag ref** (issue #513), so a signature from any other workflow does not verify; `governance-drift.yml` re-checks this against the latest real release |
| 1.14.3 | Build pipeline warns on outdated components | satisfied | govulncheck (`unit-tests.yml:360-362`), Trivy (`docker-publish.yml:1255-1262` + PR-time misconfig/secret gates `container-hardening.yml:161-196`), Grype second-opinion CVE DB (`grype.yml`, gates critical/high on main+nightly), TruffleHog git-history verified-secret scan (`trufflehog.yml`), CodeQL (`codeql.yml`), zizmor (`zizmor.yml`), Dependabot + Dependency Review (`dependency-review.yml`) |
...
| 14.2.1 | Dependencies current (checker in build) | satisfied | govulncheck (`unit-tests.yml:360-362`), Trivy (`docker-publish.yml:1255-1262`), Trivy license scan (`license-compliance.yml`, issue #361), Dependabot, Dependency Review |
