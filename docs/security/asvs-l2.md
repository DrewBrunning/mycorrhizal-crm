...
| 1.14.2 | Binary signatures / verified endpoints | satisfied | cosign-signed images + SLSA provenance (`docker-publish.yml:431-436`, `docker-publish.yml:472-477`); released images additionally carry cosign-signed portable Syft SBOMs, SPDX + CycloneDX (`docker-publish.yml:515-567`, `syft-sbom.yml`); the release APK is cosign co-signed keyless (additive to the SIGNING_* keystore) alongside `attest-build-provenance` (`docker-publish.yml:227-259`); operator-facing verification steps (issue #418) — `docs/security/release-verification.md` |
| 1.14.3 | Build pipeline warns on outdated components | satisfied | govulncheck (`unit-tests.yml:204-206`), Trivy (`docker-publish.yml:613-622` + PR-time misconfig/secret gates `container-hardening.yml:144-179`), Grype second-opinion CVE DB (`grype.yml`, gates critical/high on main+nightly), TruffleHog git-history verified-secret scan (`trufflehog.yml`), CodeQL (`codeql.yml`), zizmor (`zizmor.yml`), Dependabot + Dependency Review (`dependency-review.yml`) |
...
| 10.3.1 | Signed updates | satisfied | No auto-update feature; images are cosign-signed with SLSA provenance (`docker-publish.yml:435-436`, `docker-publish.yml:472-477`); verify commands for operators in `docs/security/release-verification.md` |
...
| 11.1.3 | Per-user limits on business actions | partial | Per-user quotas exist on the fan-out/operational surfaces (issue #415): 20 webhooks (`webhook_controller.go:34`), 5 concurrent in-memory import sessions (`services/import_session.go:29` `MaxImportSessionsPerUser`, enforced by `import_session.go:216` `CountActive` + the upload/accept handlers), 20 push subscriptions + 20 device registrations (`notification_controller.go:24-26`). No quotas on content creation (contacts/notes/activities). Gap: content quotas would be a product decision. |
...
| 14.2.1 | Dependencies current (checker in build) | satisfied | govulncheck (`unit-tests.yml:204-206`), Trivy (`docker-publish.yml:613-622`), Trivy license scan (`license-compliance.yml`, issue #361), Dependabot, Dependency Review |
