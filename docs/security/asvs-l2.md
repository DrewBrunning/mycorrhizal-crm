...
| 1.14.2 | Binary signatures / verified endpoints | satisfied | cosign-signed images + SLSA provenance (`docker-publish.yml:526-532`, `docker-publish.yml:552-560`); released images additionally carry cosign-signed portable Syft SBOMs, SPDX + CycloneDX (`docker-publish.yml:569-576`, `syft-sbom.yml`); the release APK is cosign co-signed keyless (additive to the SIGNING_* keystore) alongside `attest-build-provenance` (`docker-publish.yml:230-233,266-268`); operator-facing verification steps (issue #418) — `docs/security/release-verification.md` |
| 1.14.3 | Build pipeline warns on outdated components | satisfied | govulncheck (`unit-tests.yml:213-215`), Trivy (`docker-publish.yml:667-674` + PR-time misconfig/secret gates `container-hardening.yml:144-179`), Grype second-opinion CVE DB (`grype.yml`, gates critical/high on main+nightly), TruffleHog git-history verified-secret scan (`trufflehog.yml`), CodeQL (`codeql.yml`), zizmor (`zizmor.yml`), Dependabot + Dependency Review (`dependency-review.yml`) |
...
| 10.3.1 | Signed updates | satisfied | No auto-update feature; images are cosign-signed with SLSA provenance (`docker-publish.yml:526-532`, `docker-publish.yml:552-560`); verify commands for operators in `docs/security/release-verification.md` |
...
| 14.2.1 | Dependencies current (checker in build) | satisfied | govulncheck (`unit-tests.yml:213-215`), Trivy (`docker-publish.yml:667-674`), Trivy license scan (`license-compliance.yml`, issue #361), Dependabot, Dependency Review |
...
| 14.2.5 | SBOM maintained | satisfied | SBOM generated + attached as image referrer (`docker-publish.yml:485-490`); standalone signed SPDX/CycloneDX SBOMs per release and per main-branch merge (`syft-sbom.yml`); how to fetch and verify one — `docs/security/release-verification.md` |
