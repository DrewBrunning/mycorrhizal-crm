...
| 11.1.3 | Per-user limits on business actions | partial | Per-user quotas exist on the fan-out/operational surfaces (issue #415): 20 webhooks (`webhook_controller.go:34`), 5 concurrent in-memory import sessions (`services/import_session.go:39` `MaxImportSessionsPerUser`, enforced by `import_session.go:226` `CountActive` + the upload/accept handlers), 20 push subscriptions + 20 device registrations (`notification_controller.go:24-26`). No quotas on content creation (contacts/notes/activities). Gap: content quotas would be a product decision. |
