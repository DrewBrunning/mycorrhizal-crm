...
| Measured | Result |
|---|---|
| Path citations | 1,061 (805 + 112 + 144), of which 399 carry a line or line range |
| Test-identifier citations | 123 (Go `TestXxx` and `TestFamily_*`, Kotlin `SomeTest.method`) |
...
| Suite | Result | Scale |
|---|---|---|
| `cd backend && go build ./... && go vet ./... && gofmt -l . && go test ./...` | pass (exit 0), 92 packages | 4,657 `Test` + 8 `Fuzz` + 4 `Benchmark` functions across 675 test files |
