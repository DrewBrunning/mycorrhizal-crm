...
| An outbound HTTP client | **Partly.** INT-01 (issue #464) added `backend/integrations/matrix_test.go`: it enumerates every file under `backend/services/` that opens an outbound client and fails on one not claimed by a classification row (`TestEveryOutboundClientIsClassified`), and `TestSSRFClaimsMatchSource` fails when a row claims a `SafeDialContext`-guarded posture its source does not back up. That covers `services/` and forces the SSRF posture to be written down per client, but it is not tree-wide and does not by itself assert the guard is *wired* for a client that never claims to be guarded. The tree-wide "any outbound client anywhere must route through `SafeDialContext`" check is still issue **#609** (semgrep rule). |
