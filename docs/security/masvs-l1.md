...
| PLATFORM-2 | Validate + sanitize all external input | satisfied | The OIDC deep link is strictly validated — scheme, host, **and** path — so an exported `MainActivity` cannot be driven by a foreign explicit-component VIEW intent (`MainActivity.kt:267-281`); pinned by `OidcReturnParsingTest` (`app/src/test/.../OidcReturnParsingTest.kt:53-58`). Notification deep links are parsed into known routes only (`MainActivity.kt:248-253`). |
