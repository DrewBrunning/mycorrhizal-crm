...
**self-hosted** server (`docs/deployment.md:35`) with a certificate the app cannot know in advance —
...
| AUTH-3 | Stateless tokens signed with a secure algorithm | satisfied | The backend issues HS256 JWTs (`asvs-l2.md` 3.2.4); the client treats the JWT as an opaque bearer credential and validates it against `/users/me` before flipping to logged-in (`MainActivity.kt:265-282`). |
...
| ID | Requirement (abbrev.) | Status | Evidence |
|---|---|---|---|
| PLATFORM-1 | Minimum necessary permissions | satisfied | Permissions are runtime, opt-in, requested at the moment the user turns the feature on, and documented inline with rationale: `READ_CONTACTS` (link-action resolver, requested inline — `AndroidManifest.xml:7-12`). The call-log/SMS/phone tracking permissions (`AndroidManifest.xml:14-17`) are requested from the Settings toggles (issue #721): the DataStore opt-in flag is only ever set once the matching grant is present, a denial reverts the toggle and offers a rationale or the system app-settings deep link, and every entry into Settings reconciles each toggle against the real grant state so a revoked permission renders off rather than lying (`feature/settings/src/main/kotlin/com/mycorrhizal/crm/feature/settings/SettingsViewModel.kt:126-244`). Notifications, overlay, foreground service, boot (`:18-26`). `uses-feature ... required="false"` so tablets/ChromeOS install without telephony hardware (`:30-31`). The `play` distribution flavor omits the call/SMS capture feature entirely (issue #1200), so its merged manifest drops those permissions rather than requesting capabilities it cannot hold (`app/src/play/AndroidManifest.xml`). |
| PLATFORM-2 | Validate + sanitize all external input | satisfied | The OIDC deep link is strictly validated — scheme, host, **and** path — so an exported `MainActivity` cannot be driven by a foreign explicit-component VIEW intent, and only a callback carrying both the short-lived exchange `code` and the app's `state` nonce is accepted (`MainActivity.kt:333-352`); pinned by `OidcReturnParsingTest`, including the regression that a legacy raw-`token` link is ignored (`app/src/test/.../OidcReturnParsingTest.kt:57-87`). Notification deep links are parsed into known routes only (`MainActivity.kt:281-309`). |
| PLATFORM-3 | No sensitive functionality via custom URL schemes unless protected | satisfied | The only custom scheme is `mycorrhizal://`, and its one token-bearing route (`/oidc/callback`) no longer carries the OIDC JWT (issue #965). The callback delivers a short-lived exchange code bound to a PKCE S256 challenge the app generated; the code is redeemable only with the on-device verifier (never sent through the scheme). `AuthMiddleware` refuses any token carrying the `oidc_native_exchange` purpose claim (`backend/middleware/auth.go:109-120`). The narrow exchange handler verifies the PKCE verifier before it issues the session (`backend/controllers/oidc_controller.go:351-399`), and before redemption the app checks its own state nonce and pending-request TTL (`OidcLoginCoordinator.kt:64-128`; `OidcPkce.kt`). A stolen redirect yields an unusable code. |
...
| CODE-2 | Release-mode build, non-debuggable | satisfied | `isMinifyEnabled = true`, `isShrinkResources = true`, ProGuard rules for R8 (`app/build.gradle.kts:104-109`, `app/proguard-rules.pro`); release builds are non-debuggable by AGP default; `./gradlew :app:assembleObtainiumRelease` is gated in `.github/workflows/android-apk-build.yml` (signing secrets required, fail-fast) and `docker-publish.yml:473`. |
...
| CODE-9 | Free toolchain security features enabled | satisfied | R8 byte-code minification + resource shrink (`app/build.gradle.kts:104-105`), ProGuard rules for reflection-using libs (`app/proguard-rules.pro`), and the platform's default hardening (NX, ASLR) apply to the managed app. |
