...
| [M1](67-M1-mobile-android-app.md) · Native Android app (Kotlin, Jetpack Compose) | **Phase 1 DONE / Phase 2 DONE / Phase 3 DONE / Phase 4 DONE** (2026-08-10 — working core client in `android/`: Gradle multi-module build, JWT/API-token auth, contacts list/detail/create/edit, activities/notes/reminders + unified timeline, tappable field actions + link-action enrichment (on-device verified), local FTS search, 343 hand-verified tests, CI workflow. Phase 3 sub-resources all shipped. Phase 4 native features shipped: call/SMS tracking → interaction sync, local reminder/cadence/birthday notifications, quick-capture FGS + overlay, Settings toggles. Phase 5 (T57 import + polish) remains planned; see the ticket's landing note) |
