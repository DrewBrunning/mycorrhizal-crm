...
| [M1](67-M1-mobile-android-app.md) · Native Android app (Kotlin, Jetpack Compose) | **Phase 1 DONE / Phase 2 DONE** (2026-08-10 — working core client in `android/`: Gradle multi-module build, JWT/API-token auth against the beta backend, contact list + detail + create/edit, activities/notes/reminders with a unified timeline, tappable field actions + mobile link-action enrichment (Signal/WhatsApp/Telegram/etc. chips via ContactsContract MIMETYPE resolution), local FTS search, hand-verified tests, CI workflow. Phases 3–5 (sub-resources, call/SMS tracking, T57 import) remain planned in the same ticket; see its landing note) |
