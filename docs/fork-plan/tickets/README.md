...
| Ticket | Status |
|---|---|
| [T97](141-T97-android-status-bar-dark-mode.md) · Status bar colour inverted in dark mode | **TO BE DONE**. R4. M. Visible on every screen and every launch for any dark-mode user. Four places set bar appearance and none reads the theme; there is no `values-night/` anywhere under `android/`. Note `window.statusBarColor` may already be inert at `targetSdk = 35` — verify before designing around it. |
| [T106](150-T106-android-splash-screen-dark-mode.md) · Splash screen is always light | **TO BE DONE**. R2. S. There is no splash implementation at all — what shows is the system-generated one, derived from a `Light`-only theme. Shares the missing `values-night/themes.xml` with T97; land them together. |
