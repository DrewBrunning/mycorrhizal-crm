...
> **Beta issue triage, 2026-08-13.** A batch of 19 issues reported against the current beta, triaged one
> ticket per reported issue into **T88–T106**, plus **T107/T108** for two bugs the investigation turned up
> that nobody had reported. Eight shipped in the same session (see Done); the rest are ranked below.
>
> Four findings changed what got built, and are worth keeping visible because in each case the report and
> the code disagreed:
>
> - **"Merge doesn't carry forward circles" was a frontend bug.** `RepointContactAssociations` re-points
>   `circle_members` and `contact_tags` correctly. `ContactDetailPage` renders the same element for every
>   `/contacts/:id`, so navigating to the keeper never unmounts it and `useCircles` never refetches — the
>   keeper shows its pre-merge membership. Same root cause as the "merge modal stays open" report, so
>   [T94](138-T94-merge-dialog-stays-open.md)/[T95](139-T95-merge-keeper-shows-stale-circles.md) shipped
>   together.
> - **The `"private"` address type was never Android's bug.** `contactAddressFromNeutral` copied
>   `Contexts[0]` into the flat `Type` untranslated, so *every VCF-imported address on web* had it too.
>   Fixed in the projection ([T91](135-T91-imported-address-type-private.md)), which fixes both clients at
>   once; the Android mapper was correct and was left alone.
> - **"Mark a contact as Me" already exists in the schema.** `users.self_contact_vcard_uid` (migration
>   `000018`) is written only at registration, has no setter endpoint, and the lazy backfill its own model
>   comment promises does not exist — so every pre-`000018` account is `NULL` forever.
>   [T90](134-T90-mark-contact-as-me.md).
> - **Merge silently destroys four association types** — attachments (including the files on disk),
>   preferences, cadence policies and external identities/activities — and none appears in the preview
>   counts. Unreported, rating 5: [T107](151-T107-merge-destroys-attachments-and-more.md). It should land
>   before [T92](136-T92-bulk-merge-from-contacts-list.md) makes merge easier to reach.
>
> Three of the reported issues (bulk merge, duplicate scanning, import-merge review) all need the same
> missing duplicate-detection endpoint. They stayed as separate tickets per the one-ticket-per-issue
> decision, with [T93](137-T93-duplicate-scan-endpoint-and-review.md) owning the engine and the other two
> forbidden from re-specifying their own.
>
> [T104](148-T104-suggest-relationships-from-relationships.md) is filed **deliberately unready** — the
...
| | **→ beta issue triage, 2026-08-13 — the eight below shipped in one session** |
| [T94](138-T94-merge-dialog-stays-open.md) · Merge dialog stays open after a successful merge | **DONE** (2026-08-13 — `handleCommit` closes and resets before calling `onMerged`, and `ContactDetailPage` also owns its own dialog state so the second entry point T92 adds can't reintroduce this. Failure deliberately does not close, so a retry keeps the user's conflict answers. Two new tests; the success case hand-verified to fail when the close is removed) |
| [T95](139-T95-merge-keeper-shows-stale-circles.md) · Merge keeper shows stale circles | **DONE** (2026-08-13 — the reported "merge doesn't carry forward circles" was `useCircles`/`useTags` never refetching after the navigate, not a backend bug. `onMerged` now refreshes both. The backend already had a repoint test, but it seeds the membership on the *loser only*, so the dedup leg had never executed — new `TestContactMerge_SharedCircleAndTag_Deduped` covers a shared circle+tag alongside a loser-only one, hand-verified against removing each of the DELETE and the UPDATE) |
| [T89](133-T89-circles-tags-edit-icon-placement.md) · Circles/tags edit pencil sits at the far right | **DONE** (2026-08-13 — `ml: 'auto'` → `ml: 1, flexShrink: 0`, matching the Name field. The pencils were already hover-revealed like Name; only the horizontal placement differed) |
| [T102](146-T102-archive-button-color-consistency.md) · Archive button colour inconsistent | **DONE** (2026-08-13 — all four archive surfaces now match, and the archived chip goes neutral. Establishes the rule T62 never issued: **reversible** state changes are neutral outlined; `warning`/`error` are for destructive or irreversible ones) |
| [T98](142-T98-web-activity-title-nav-label-size.md) · Activity titles and nav labels too small on web | **DONE** (2026-08-13 — all four timeline title renders moved to `subtitle1` together so a merged list can't carry mixed sizes; Activities/prep-view titles too, dropping a `color` override that only existed to undo `body2`'s themed colour; drawer labels gained an explicit `fontSize`, with `drawerWidth` 180 → 256 so that no nav label wraps in any of the five locales — which was never true before, four of them wrapped even at the old size. No shared theme variant touched, per T63) |
| [T91](135-T91-imported-address-type-private.md) · Imported addresses always typed "private" | **DONE** (2026-08-13 — **not an Android bug**: the ctx2type table was `vcard4`-private, so the Card→flat projection shipped the untranslated neutral token for *every* VCF-imported address on web too. Table moved to `contactmodel`, which both packages already import; projection translates with a deliberate fall-through for unmapped contexts. End-to-end test spans `vcard4.Import` → `ApplyRecordToContact`, the boundary that hid it. Live verification then caught that the fix was half done: the detail page reads the nested Card, not the flat column, and `cardAddressesToValues` had the identical untranslated line — fixed there too. No data migration — existing `"private"` rows display as raw text but aren't lost, and a blanket JSON UPDATE on production data is the bigger risk) |
| [T99](143-T99-android-drop-eb-garamond.md) · Drop EB Garamond from Android | **DONE** (2026-08-13 — Android now has zero serif usage, a deliberate divergence from web's T63 recorded in the font object's own doc comment so the next parity audit doesn't re-file it. `ThemeTest`'s pin was inverted rather than deleted. `THIRD_PARTY_SOURCES.md` deliberately **not** touched — web still bundles the face. Not on-device verified) |
| [T100](144-T100-android-drawer-label-size.md) · Android drawer labels too small | **DONE** (2026-08-13 — `fontSize = 16.sp` on the two scoped `labelLarge.copy(...)` calls, never on the shared role, which is also M3's Button and Snackbar default. Same commit as T99, which edits the same two calls. Not on-device verified) |
| [T105](149-T105-coworker-relationship-type.md) · Add `coworker_of` | **DONE** (2026-08-13 — registry + both frontend mirrors + Android's `RelationshipEdgeTypes` + five locales. `Registries.kt`'s duplicate `RelationshipType` object was **deleted** rather than updated: grep confirmed zero consumers, and a second hand-synced mirror with nothing reading it is the exact drift hazard trap #4 warns about) |
| [T107](151-T107-merge-destroys-attachments-and-more.md) · Merge destroys attachments, preferences, cadence & external links ⚠ | **DONE** (2026-08-13 — attachments/preferences/external identities/activities all now re-point instead of being deleted; the pre-transaction attachment-file pluck was removed outright, not reordered, since nothing is deleted by a merge anymore. `cadence_policies` is the one real new conflict type, reusing `ContactMergeResolution.Conflicts` (zero frontend changes — `MergeContactsDialog.tsx`'s existing generic radio UI renders it). The keeper adopts the loser's photo when it has none, via the same flat-field-mutation-then-`BeforeSave` pattern every other field already uses. Design correction made mid-implementation: `external_identities`/`external_activities`' unique index doesn't include `entity_id`, so it constrains per-user, not per-contact — a same-key collision between keeper and loser can't exist to begin with, proven when a dedupe-path test failed at fixture `Create`; the dead dedupe branch was removed, plain repoint only. Six new/extended real-DB tests cover every table `deleteContactAssociations` touches; every new assertion hand-verified by temporarily disabling the fix and confirming the matching test failure. `openapi.yaml` updated. A self-review pass (8 parallel finder angles) found and fixed 4 real bugs in the same session — a silent-fallthrough on a stale cadence-conflict resolution, order-sensitive `QualifyingTypes` comparison, a wrong error message on the commit path, and a frontend TS-type gap — see the ticket's own landing note. Also fixed an unrelated flaky CI test (`TestLifeEventETagSaveDoesNotLoop`/`TestContactETagSaveDoesNotLoop`/`TestActivityETagSaveDoesNotLoop`, a real-clock second-boundary race) found while chasing this branch's CI failure) |
| [T108](152-T108-contact-summary-missing-columns.md) · `GET /contacts` returns empty nickname and circles | **DONE** (2026-08-14 — `nickname` added to `contactSummaryColumns`; `circles` removed from `ContactSummary` entirely rather than populated, since it would only ever have shipped the stale legacy flat column T2/T3 superseded. New raw-JSON regression test hits the real handler and asserts against the parsed JSON map — catches both the nickname value and a stray `circles` key a decoded-struct test can't see. The pre-existing unit test for this exact gap had never caught it because it called `NewContactSummary` directly, never through the controller's actual `Select()` column list where the real bug lived — left narrowed to its legitimate mapping-logic purpose, with a comment pointing at the new controller-level test. `openapi.yaml` and the frontend DTO already matched the target shape; the frontend just gained the "hand-synced mirror" comment) |
| [T90](134-T90-mark-contact-as-me.md) · No way to mark a contact as "Me" | **DONE** (2026-08-14 — `PATCH /users/me/self-contact` set/clear with ownership-scoped 404 (the uuid must resolve to a non-deleted contact owned by the caller); the promised lazy backfill finally exists — `GetCurrentUser` calls `EnsureSelfContact` unconditionally (idempotent, failure logged not 500'd), so every pre-`000018` account gains a "Me" contact on its next `/users/me`. The two dangling-pointer hazards are pinned: `DeleteContact`/`deleteContactAssociations` nulls the pointer when the deleted contact was "Me", and `RepointContactAssociations` moves it onto the keeper when "Me" is the merge loser. Web: a Settings "Your contact" picker (debounced `getContacts` autocomplete + clear, cache refreshed after PATCH) and a neutral `color="default"` "You" chip on the contact header (per T62 — being yourself is not a status condition) with "This is me / This isn't me" in the compact overflow menu. All new strings in five locales. Real-migrated-schema controller tests (backfill, idempotence, set/clear/foreign-uid/unknown-uid/malformed-uid, delete-null, merge-repoint through the commit path) + merge repoint tests + ContactHeader/SettingsPage/api/auth unit tests, hand-verified; OpenAPI updated. Review pass hardened `EnsureSelfContact` to be transactional (a failed pointer-write can't orphan a contact) and simplified the web toggle to optimistic + one cache refresh) |
| [T93](137-T93-duplicate-scan-endpoint-and-review.md) · Nothing can scan the address book for duplicates | **DONE** (2026-08-14 — new `GET /contacts/duplicates` returns candidate **pairs** (each side a `ContactSummary`, full `reasons` set, `confidence` heuristic) strongest-first with offset pagination + `total`; detection is set-wide SQL grouping over columns `BeforeSave` already maintains — one window-function query per tier (email / exact firstname+lastname with a no-contact-info exclusion for the false-positive name tier / phone via `phones_normalized` tokenization, which finds shared **non-primary** numbers `DetectDuplicate` cannot) plus one summary fetch and one dismissal fetch, **exactly 5 queries pinned by a counting-logger test** against 150 seeded contacts. `POST /contacts/duplicates/dismiss` writes a hard-delete `dismissed_duplicate_pairs` row keyed on the ordered uid pair (migration `000023`, idempotent, ownership-scoped 404), swept by both `deleteContactAssociations` and `DeleteUser`. Web: a "Review duplicates" button on the Contacts page opens a review dialog listing pairs with per-pair Merge (opens `MergeContactsDialog` in a new pair mode with an explicit keeper radio + swap — the T92-step-5 lift, done here so this ticket isn't blocked on it) and persistent Not-a-duplicate dismissal. 2 real-DB controller tests (all three tiers incl. T68 phone case + archived flag + cross-user isolation + dismissal persistence + delete-cascade cleanup; raw-JSON `[]`-not-`null` pin) + query-count pin + 4 vitest component tests + a Playwright spec driving dismiss→reopen-persistence→merge against the real UI; full 171-test e2e suite green against a rebuilt Docker test stack. OpenAPI updated. One finding worth keeping: a merge POST returning 200 is not immediately visible to a follow-up GET through SQLite's pooled WAL connections (a read snapshot can briefly serve the pre-commit state) — the e2e polls the loser's 404 rather than asserting a single request. **Post-landing self-review pass 2026-08-14 fixed all findings:** the name tier now groups by the lowercased name TUPLE rather than a `|`-joined string (the old key could collapse `"A|B"+"C"` with `"A"+"B|C"` into a false pair — collision test added and hand-verified to fail against the old code); dismissal idempotency is race-safe via unique-index tolerance instead of a racy count-then-create (8-way concurrency test, all 200 + exactly one row); `duplicateSummaryColumns` consolidated into `models.ContactSummaryColumns` so the service and `GetContacts` can't drift; `MergeContactsDialog`'s props are a discriminated union (no mixed-mode call sites), pair mode issues exactly one preview, and the keeper radio group is accessibility-labeled; the review dialog fills incrementally across pages; see the ticket's landing note) |
| [T88](132-T88-contact-info-two-column-holes.md) · Contact-info two-column layout collapses | **DONE** (2026-08-14 — `how_we_met`/`contact_information` unwrapped from `FullSpanField` so they pair up; both grid tracks changed to `repeat(2, minmax(0, 1fr))` for the floor. Pronouns-full-span-only-while-editing needed more than the ticket's one-liner: naively swapping between the bare field and a `FullSpanField`-wrapped one changes the element type at that tree position exactly when `EditableArrayField`'s own internal `editing` state flips true, so React discards and remounts a fresh instance mid-click, silently resetting `editing` back to false — clicking the edit button appeared to do nothing. Fixed by always rendering the same wrapper and toggling only its `sx` via a new `active` prop, plus a new `onEditingChange` callback on `EditableArrayField` so the parent can observe the internal state at all. New tests extract the actual generated full-span class name(s) from the stylesheet and walk a bounded number of DOM ancestors from a field's text node, since the existing coarse "does `grid-column:1/-1` appear anywhere" checks can't isolate one field's span from cardNotes/work_information's unconditional one. All three production fixes hand-verified by reverting each and confirming the matching test failure. Live-browser re-verification skipped — an unrelated local dev-backend snag, not worth chasing for a change already pinned precisely at the unit level) |
| [T101](145-T101-merge-picker-strict-match.md) · Merge picker returns non-matching contacts | **DONE** (2026-08-14 — `MergeContactsDialog`'s single-mode `Autocomplete` now filters with MUI's `createFilterOptions({ stringify: getOptionLabel })` instead of `filterOptions={(x) => x}`, so a row only shows when the typed string appears (case-insensitive) in the displayed name. Server query and `limit: 100` untouched, so a phone/email/address search still reaches the right contact. When the strict filter hides server rows, a caption reports the count ("N shown, M matched on other fields") rather than silently dropping them. New strings in all five locales. Pair mode and the AppBar autocomplete are untouched. Hand-verified against a real backend+browser: a contact matched only via its address city was correctly hidden from the picker with the count shown, and the AppBar dropdown's pre-existing (unrelated) default filtering was confirmed unaffected) |
| [T96](140-T96-import-duplicate-merge-review.md) · Import duplicate/merge review (web + Android) | **DONE** (2026-08-14 — the import review step is now per-contact decision cards on web **and** both Android flows: Match line + backend merge diff ("what Merge will change"), Merge / Keep Both / Discard New buttons (Merge only when the row matched an existing record), "Resolve all as merged" + "Apply Decisions (N)", and a "Resolve Conflicts (N remaining)" heading. Backend: `ImportRowPreview` gains `merge_diff` (computed by `ComputeImportMergeDiff` from the exact helpers `MergeImportedContact` applies — a convergence test proves the diff predicts the merge, so the preview can never lie) and `batch_duplicate_of` (T96 step 3's within-batch detection, defaulting twins to Discard); `updated`/`added` always serialize `[]`, never null (trap #8, raw-JSON pinned). New `POST /contacts/import/records` feeds Android's device-contacts flow through the same server-side pipeline (select → review → confirm) instead of creating unconditionally — the device LOOKUP_KEY linkage is deliberately dropped. Frontend `ImportContactsDialog` review redesigned (cards, not a table-select); 21 Android strings + web i18n ×5. Tests: pure diff/batch units + real-schema ParseVCF wiring + records-endpoint controller tests, rewritten dialog vitest suite, new `importMergeReview.spec.ts` e2e (merge/keep-both/discard/within-batch), Android ViewModel + review-UI tests; full gates green incl. the 179-test Playwright suite. See the ticket's landing note) |
| [T97](141-T97-android-status-bar-dark-mode.md) · Status bar colour inverted in dark mode | **DONE** (2026-08-14). Added `values-night`/`values-night-v27` theme resources; `MycorrhizalApp.kt`/`ContactDetailScreen.kt` now read `MaterialTheme.colorScheme.*` instead of hardcoded `MycorrhizalColors`, with `isAppearanceLightStatusBars` derived from theme + color role (new `LocalDarkTheme`, mirroring `LocalDrawerOpen`); `MainActivity`'s `enableEdgeToEdge()` now passes explicit `SystemBarStyle` scrims; fixed the dispose-time race with the open drawer. Landed together with T106 (shared `values-night/themes.xml`). `testDebugUnitTest`/`lintDebug`/`assembleDebug` green. On-device verified on a Pixel 8a across both themes × drawer open/closed × contact-detail collapsed/expanded — see the ticket's landing note. |
| [T106](150-T106-android-splash-screen-dark-mode.md) · Splash screen is always light | **DONE** (2026-08-14). Added `androidx.core:core-splashscreen:1.2.0`, a `Theme.Mycorrhizal.Splash` (light + `values-night` dark background, reusing the launcher foreground as the icon, `postSplashScreenTheme` → `Theme.Mycorrhizal`), `installSplashScreen()` before `super.onCreate()`, and `values-night/colors.xml` for the launcher-icon background. Landed together with T97 (shared `values-night/themes.xml`). `testDebugUnitTest`/`lintDebug`/`assembleDebug` green. On-device verified on a Pixel 8a via screen-recorded cold launches in both themes (dark: `boneDark` background throughout, no white flash; light: unchanged) — see the ticket's landing note. |
| [M21](103-M21-android-relationships-depth.md) · Relationships depth | **DONE** (2026-08-14, implemented 2026-08-12). Name resolution, search-based linking, edit, sensitivity, gender/birthday, reject-vs-delete, and confirmed/suggested sectioning all landed; `testDebugUnitTest`/`lintDebug`/`assembleDebug` green. On-device verified on a Pixel 8a against real account data: relationship rows resolve to real contact names (not raw UIDs) and are tappable/navigable, the edit dialog round-trips type/sensitivity, and the create dialog's "Link to contact" mode shows the search field replacing manual UID entry. Not observed this pass: a `suggested` (household-inferred) edge — the account's current relationship data is all `confirmed`, so the reject-vs-delete label distinction and the confirmed/suggested section split weren't exercised live (code path unchanged since the 2026-08-12 landing note's test coverage). |
| [M9](91-M9-android-wire-up-existing-screens.md) · Wire up already-built screens & dead code | **DONE** (2026-08-14, implemented 2026-08-13). All four items landed: global Notes/Activities drawer inboxes, bulk circle/tag picker actions, scroll-triggered contact-list pagination, and a VCF-file import flow (built despite the M8 sign-off's CSV/VCF exclusion — overridden for the VCF-only path on explicit instruction; CSV file import stays excluded). Also fixed a real bug found along the way: `ActivitiesPage.activities` crashed on Moshi parse for a zero-activities response (`/CLAUDE.md` trap #8), never caught before because nothing called `listActivities()` until this ticket. `testDebugUnitTest`/`lintDebug`/`assembleDebug` green. On-device verified on a Pixel 8a: the drawer's Notes/Activities entries open real inbox screens (not the placeholder), Bulk operations shows working circle/tag pickers alongside archive/unarchive/delete, the contacts list scrolls through several dozen distinct real contacts with no stall or page-1 repeat, and Import contacts shows the "Import from a VCF file" entry point. |
