...
| Ticket | Status |
|---|---|
| [T93](137-T93-duplicate-scan-endpoint-and-review.md) · Duplicate-scan endpoint + review surface | **TO BE DONE**. R4. L. The one missing capability behind three separate beta complaints; unblocks T92's suggested-pairs half and T96. |
...
| [T103](147-T103-contacts-list-has-contact-info-filter.md) · Contacts list: default to contacts with contact info | **DONE** (2026-08-13 — `?has_contact_info=true|false` on `GET /contacts` (`contactInfoClause`: flat `email`/`phone` or any non-empty `emails`/`phones`/`urls` JSON entry), strict true/false-only validation (400 otherwise), and a `hidden_count` field present only while the filter is active — counted over the same archive/search/circle scope WITHOUT the predicate or the resume cursor, so the web client can render "N contacts without contact info are hidden" and a default-on filter never reads as lost data. The `?since=` feed and `?vcard_uid=` batch paths are deliberately unaffected (a feed is sync state). Web: a "Show all" switch beside the archived toggle, default-off (the filter defaults on), persisted in the URL as `?has_contact_info=` via the T77 pattern; it was added to the selection-clearing dependency array per the ticket's trap. Query plan measured on 20k real-schema rows: the predicate costs ~120µs on the page query (453µs vs 336µs baseline) and the hidden-count query is ~18ms — no denormalized column needed, escape hatch recorded and unused. Seven existing e2e specs that search/create no-contact-info contacts were updated to pass `has_contact_info=false` (or gained a contactable fixture where the redirect target can't carry the param); two of them had assertions that had been passing vacuously via the "No results" line. A review pass found and fixed a NULL-scalar `hidden_count` undercount (`COALESCE(email,''/phone,'')` on the flat legs — `NOT(NULL)` was NULL, so a NULL-scalar stub was excluded from the list but not counted as hidden), pinned by a raw-SQL regression test that fails pre-fix; added tests for filtered-set cursor pagination (whole-set `hidden_count` on every page), `sort=name` + filter, and the `includes=` response branch, plus two more e2e specs (fresh shared-link load; toggle clears bulk selection — the ticket's trap). 12 backend tests + 7 vitest cases + 4 Playwright specs, all hand-verified green) |
