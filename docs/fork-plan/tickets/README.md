...
| [T69](113-T69-phone-search-tokenization.md) · Phone search misses results because nothing normalizes phone numbers | **TO BE DONE**. R3. M. Re-scoped 2026-08-11: there are **three** phone-search paths, not one — FTS5 global search, the contacts-list `LIKE` search bar, and Android's own index ([T76](120-T76-android-local-fts-phone-search.md)). This ticket covers the two backend ones, plus the gap that `contacts_fts` indexes only the *primary* phone. Approach decided: a normalized shadow column following T38/migration `000010`'s proven template. |
