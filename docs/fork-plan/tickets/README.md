...
| [T82](126-T82-audit-snapshots-miss-nested-contact-data.md) · Audit snapshots never capture Card/CRM/Passthrough | **TO BE DONE** (unblocked 2026-08-12 — [T75](119-T75-plain-save-destroys-card-only-data.md) landed, making undo non-destructive; this makes it complete). R3. M. Split from T75 on 2026-08-11. `Card`/`CRM`/`Passthrough` are `json:"-"` and `auditBeforeSave` uses `json.Marshal`, so T18's history has **never** recorded nested contact data — editing a pronoun writes an audit event with no visible change — and T60's undo can't restore it. The work is the decisions around it: snapshot size on the most-updated entity in the product, and whether `private`/`secret` data belongs in a snapshot that outlives its own deletion. |
