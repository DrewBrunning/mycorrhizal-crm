...
| Ticket | Status |
|---|---|
| [T85](129-T85-contacts-list-fts-search.md) · `GET /contacts?search=` should use FTS5, so search composes with the list's filters | **TO BE DONE**. R4. S–M. Filed 2026-08-12 out of the Contacts/Search fold design pass. Two engines exist — `applyContactSearch`'s LIKE scan (filterable, sorted, cursor-paginated) and `services.Search`'s FTS5 (better matching, cross-entity, top-50 only) — and neither can do the other's job. Resolved by using FTS as a **filter, not a ranker**: `contacts_fts` narrows the row set, the existing `sort`/`order`/cursor still order it, so bm25 never has to back a keyset cursor. `OR`-ed with the existing LIKE clause so the param's three live consumers (web list, web AppBar, Android list) only ever gain matches. No migration. Blocks [T86](130-T86-web-fold-search-into-contacts.md) and [T87](131-T87-android-fold-search-into-contact-list.md). |
...
| [T77](121-T77-web-contacts-list-sort-control.md) · Contacts page has no sort control | **TO BE DONE**. R3. S. Split out of T73 on 2026-08-11; unblocked 2026-08-12 when T73's backend landed. Defaults to alphabetical on web while the server keeps `updated_at` for existing API consumers. |
| [T86](130-T86-web-fold-search-into-contacts.md) · Fold the Search page into Contacts: one search field, one list | **BLOCKED** on [T85](129-T85-contacts-list-fts-search.md). R3. M. Filed 2026-08-12; promoted out of Deferred → Feature ideas once its design pass was done. Web has **three** search surfaces (AppBar autocomplete, `/search`, and the Contacts list) and Contacts is already a search-results page with no search field — it reads `?search=` but nothing on it can write it. Decisions made: field owns `?search=`, notes/activity hits render in a collapsed section below the list (`/search`'s contacts group discarded so two contact lists never disagree), `/search` redirects and `SearchPage.tsx` is deleted, the AppBar box **stays** as jump-to-contact with its Enter retargeted. Land [T77](121-T77-web-contacts-list-sort-control.md) first — same filter row, same URL-param question. |
| [T80](124-T80-web-address-editor-line-two.md) · Address editor has no line 2 / PO box / floor field | **TO BE DONE** (unblocked 2026-08-12 when T79 landed). R3. S. `AddressFields.tsx` renders five inputs, so an apartment has nowhere to go but the street line. The flat projection now carries the data (T79) — this ticket is the editor inputs. Watch the interaction with [T74](118-T74-desktop-field-row-action-distance.md) — a grown address editor renders in a ~530px column once that lands. |
| [T78](122-T78-web-timeline-bounded-view-explorer.md) · Contact timeline: 5-item default + "View all" explorer | **TO BE DONE** (unblocked 2026-08-12 when T66 landed). R3. M. Split out of T66 on 2026-08-11. Filter vocabulary (six types; 7/30/90/this-year/all buckets) is fixed by T66 — match it exactly; the paginated endpoint it consumes is now live. |
...
| [T87](131-T87-android-fold-search-into-contact-list.md) · Fold search into the contact list, retire the placeholder search route | **BLOCKED** on [T85](129-T85-contacts-list-fts-search.md). R4. S–M. Filed 2026-08-12 alongside [T86](130-T86-web-fold-search-into-contacts.md) so the fold doesn't land on web and leave Android on the old IA. **Supersedes [M13](95-M13-android-real-search.md)** — same capability, different destination: M13 specifies a screen mirroring `SearchPage.tsx`, which T86 deletes. Android gets the contact-matching half free from T85 (its list already calls `GET /contacts?search=`); what remains is the cross-entity half — one `ApiClient.search` method, a collapsed notes/activities section, and deleting the drawer placeholder. Carries one Android-only decision web doesn't have: offline, the section is hidden rather than errored, and is never routed into the local FTS4 contact mirror. |
