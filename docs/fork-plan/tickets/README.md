...
| [M1](67-M1-mobile-android-app.md) · Native Android app (Kotlin, Jetpack Compose) | **Phase 1 DONE / Phase 2 DONE / Phase 3 DONE** (2026-08-10 — working core client in `android/`: Gradle multi-module build, JWT/API-token auth, contacts list/detail/create/edit, activities/notes/reminders + unified timeline, tappable field actions + link-action enrichment (on-device verified), local FTS search, 337 hand-verified tests, CI workflow. Phase 3 sub-resources all shipped: circles, tags, households, relationship edges, life-events/gifts/preferences/agenda, contact merge + bulk ops. Phases 4–5 (native call/SMS, quick-capture, notifications, T57 import + polish) remain planned; see the ticket's landing note) |
