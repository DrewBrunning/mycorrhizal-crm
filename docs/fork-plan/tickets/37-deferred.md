...
These tickets are **not implementation-ready and are not meant to be.** Each needs its own design
pass before it can be broken into work packages. The sections above describe what a design pass
would need to settle, not what to build. If one of these becomes a concrete need, open a new
ticket from the design pass — do not attempt to implement from this file.
