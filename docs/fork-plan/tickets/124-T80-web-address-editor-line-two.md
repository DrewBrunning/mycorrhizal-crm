...
| **Status** | **DONE**, 2026-08-12. T79's landing already carried the frontend flat type (`ContactAddress.pobox/apartment/floor`), the card⇄flat adapters, `formatAddressLine`, and `EMPTY_ADDRESS`'s initializer — so items 1 and 3 of "What to build" were already done; this ticket was purely `AddressFields.tsx`'s editor inputs plus i18n. Added the three inputs after Street, hidden by default behind an "Additional fields" button and auto-revealed (no toggle rendered at all) whenever any of them is non-empty — reveal state is tracked per-address by `useRowKeys`' stable row key (not array index), in a `Set` that only ever grows, so it survives clearing the fields back to empty and one address's reveal never affects another's. New `contacts.addressFields.{pobox,apartment,floor,additionalFields}` keys, all five locales. Hand-verified in the browser: entered a full address (street/PO box/apartment/floor/city/region/postal), saved, reloaded, and reopened the editor — all three extra fields were pre-expanded with the round-tripped values, no extra tap; added a second empty address alongside it and confirmed only the second one showed the reveal button (the first stayed expanded); no horizontal overflow at 390px. `cd frontend && npx tsc --noEmit && npx vitest run` green (632/632). Landed as part of the T71/T72/T78/T65 web rollup branch. |
