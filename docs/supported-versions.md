...
| **Expected to work** | The floor is enforced at build time (or by the platform) but no CI job drives it every release | Safari/iOS ≥ 16.4 (a PR/nightly `e2e-webkit-smoke` job app-load smoke-tests the *current* WebKit engine, per issue #992 — there is no downloadable old-WebKit build to pin the exact 16.4 floor against, and the Push API itself is unverifiable in any CI-available WebKit), host OS/docker daemon behavior on a network filesystem |
