...
- Distribution note (issues #1133, #1200): the app builds three distribution flavors from one source tree — `obtainium` (the gold-standard GitHub Release/Obtainium build), `foss` (F-Droid; no Firebase/GMS), and `play` (Google Play). The `play` flavor omits the call/SMS capture feature entirely (`app/src/play/AndroidManifest.xml` removes the `READ_SMS`/`RECEIVE_SMS`/`READ_CALL_LOG` permissions and the capture components; its `call_sms_tracking_available` resource is false, hiding the Settings toggles and skipping the capture workers), because Google Play restricts those permissions to default dialer/SMS apps. The `obtainium`/`foss` builds keep the full feature. See [docs/adrs/0022-distribution-variants.md](docs/adrs/0022-distribution-variants.md) and the [F-Droid submission runbook](docs/development/fdroid.md). The Play App Bundle can be produced on demand by the manual [`android-aab-build.yml`](.github/workflows/android-aab-build.yml) workflow; wiring it into the release is tracked as issue #1206.
...
- The app's `benchmark` build type inherits the release network security config, which forbids cleartext. Because the benchmark backend is plain HTTP, [android/app/src/benchmark/res/xml/network_security_config.xml](android/app/src/benchmark/res/xml/network_security_config.xml) overrides it with the same loopback (`10.0.2.2` / `127.0.0.1` / `localhost`) cleartext carve-out the debug source set has. Without it the app's login is blocked by `CleartextNotPermittedException` and the dashboard scenario fails with "dashboard did not load". `BenchmarkNetworkSecurityConfigTest` pins it; the `benchmark` variant is never distributed (release APKs still use the system-CA-only, cleartext-forbidden config).
- Run everything:
  ```bash
  docker compose -f docker-compose.test.yml up -d --build
  cd android
...
**Pen-test / demo environment (issue #849)**
- A one-command, reproducible instance of the shipped image pre-populated with realistic multi-user data (contacts, relationships, notes, activities, mixed sensitivities, soft-deletes, duplicates) — every seeded user a working login, one an admin, credentials written to the data volume. For hands-on penetration testing and for documentation screenshots.
  ```bash
